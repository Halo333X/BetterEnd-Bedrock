...
class pillar_Manager {
    ...
    static updatePillar(block) {
        ...
        if (aboveBlock != undefined) {
            ...
            } else block.setPermutation(block.permutation.withState("pillar_template:top_bit", false));
        } else block.setPermutation(block.permutation.withState("pillar_template:top_bit", false));
        if (belowBlock != undefined) {
            ...
            } else block.setPermutation(block.permutation.withState("pillar_template:bottom_bit", false));
        } else block.setPermutation(block.permutation.withState("pillar_template:bottom_bit", false));
    }
}
